...
foreach( $arr as $line ){
    ...
    $arrayElem;
    if( count($col) >= 10 && $col[1] != "kbmemfree" ){
        ...
        $arrayElem["kbcommit"] = $col[6];
        $arrayElem["commit"] = $col[7];
        $arrayElem["kbactive"] = $col[8];
        $arrayElem["kbinact"] = $col[9];
